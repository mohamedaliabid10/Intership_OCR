...
class TextSystem(object):
    ...
    def __call__(self, img_or_path: Union[str, np.ndarray], do_visualize=True):
        ...
        fn = (
            os.path.basename(img_or_path).split(".")[0]
        ...
        logger.info(
            f"Num detected text boxes: {len(polys)}\nDet time: {time_profile['det']}"
        )
        ...
        for i in range(len(polys)):
            ...
            cropped_img = crop_text_region(
                data["image_ori"], poly, box_type=self.box_type
            )
            crops.append(cropped_img)

            if self.save_crop_res:
                cv2.imwrite(
                    os.path.join(self.crop_res_save_dir, f"{fn}_crop_{i}.jpg"),
                    cropped_img,
                )
...
def main():
    ...
    for i, img_path in enumerate(img_paths):
        ...
        boxes, text_scores, time_prof = text_spot(
            img_path, do_visualize=args.visualize_output
        )
    ...
    logger.info(
        f"Done! Results saved in {os.path.join(save_dir, 'system_results.txt')}"
    )
